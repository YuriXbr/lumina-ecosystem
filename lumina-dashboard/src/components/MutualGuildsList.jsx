      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {guild.hasBot && guild.botConfig && (
            <div className="flex flex-wrap gap-1 mb-3">
              {guild.botConfig.welcomeEnabled && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-blue-50 text-blue-700">{t('guild.welcome')}</span>
              )}
              {guild.botConfig.moderationEnabled && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-yellow-50 text-yellow-700">{t('guild.moderation')}</span>
              )}
              {guild.botConfig.musicEnabled && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-50 text-purple-700">{t('guild.music')}</span>
              )}
              {guild.botConfig.memberCount > 0 && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-600">
                  {guild.botConfig.memberCount.toLocaleString(undefined)} {t('common.members')}
                </span>
              )}
            </div>
          )}
          ...
          <div className="flex gap-2">
            ...
            {!guild.hasBot && !guild.canManage && (
              <span className="inline-flex items-center gap-1 px-3 py-1.5 text-xs text-gray-500 italic">
                {t('members.servers.noPermissionAdd')}
              </span>
            )}
          </div>
        </div>
...
export default function MutualGuildsList() {
  ...
  if (error && guilds.length === 0) {
    return (
      <ErrorState
        title={t('members.servers.loadError')}
        message={error}
        onRetry={load}
      />
    );
  ...
  const filterButtons = [
    { id: 'all', label: t('members.servers.all') },
    { id: 'with-bot', label: t('members.servers.withBot') },
    { id: 'without-bot', label: t('members.servers.withoutBot') },
    { id: 'manageable', label: t('members.servers.manageable') },
  ];
    ...
    <div>
      ...
      {error && guilds.length > 0 && (
        <div className="mb-3 bg-red-50 border border-red-200 rounded-md px-3 py-2 text-xs text-red-700 flex items-center gap-2">
          ...
          <span>{t('admin.metrics.updateFailed', { error })}</span>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl p-10 text-center">
          ...
          <p className="text-sm text-gray-500">{t('members.servers.empty')}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {filtered.map(g => (
