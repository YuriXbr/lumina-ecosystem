...
const SECTION_FIELDS = {
  ...
  moderacao: ['warnsToMute', 'warnsToTimeOut', 'warnsToKick', 'warnsToBan', 'persistentMute', 'autoWarnPunishment'],
};
      ...
      <div className="space-y-5">
        ...
        {SECTIONS.map(section => {
          ...
          return (
            ...
            >
              {renderSection(section.id, config, update, { disabled: !canEdit })}
            </Accordion>
          );
        })}
      </div>
...
function renderSection(id, config, update, opts) {
  switch (id) {
    ...
    case 'automsg':   return <AutoMsgSection   config={config} update={update} opts={opts} />;
    case 'bloqueios': return <BloqueiosSection config={config} update={update} opts={opts} />;
    case 'moderacao': return <ModeracaoSection config={config} update={update} opts={opts} />;
    default:          return null;
  }
}
...
function AutoMsgSection({ config, update, opts }) {
...
// ─── 6. Moderação ───────────────────────────────────────────────────────────
function ModeracaoSection({ config, update, opts }) {
  return (
    <div className="space-y-5 pt-4">
      <div>
